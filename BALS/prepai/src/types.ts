...
// A single question-focus option generated from the candidate's own resume,
// e.g. "Ask about my internship at Deloitte" — shown in the dropdown once
// the resume has been parsed.
export interface InterviewFocusOption {
  id: string;
  label: string;
  instruction: string;
}

// Structured extraction of the uploaded resume, produced once and reused
// for every question in the session so the AI can reference specifics
// (project names, companies, skills) instead of asking generically.
export interface ResumeSummary {
  candidateName: string;
  headline: string;
  skills: string[];
  projects: { name: string; description: string }[];
  experience: { company: string; roleTitle: string; description: string }[];
  education: string[];
  focusOptions: InterviewFocusOption[];
}
